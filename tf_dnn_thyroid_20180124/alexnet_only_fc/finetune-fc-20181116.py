...
filewriter_path = 'I:/thyroid-all-gray-20171213/trainData/ChoseForBenignManign-Alex-227/modelPath-20181116/finetune_alexnet_fc'\
                  + '_' + str(weightNegative) + '_' + str(base_lr) + '' + '/tensorboard'
checkpoint_path = 'I:/thyroid-all-gray-20171213/trainData/ChoseForBenignManign-Alex-227/modelPath-20181116/finetune_alexnet_fc'\
                  + '_' + str(weightNegative) + '_' + str(base_lr) + '' + '/checkpoints'
...
validation_init_op_good = iterator.make_initializer(val_data_good.data)
validation_init_op_bad = iterator.make_initializer(val_data_bad.data)

### 计算step
train_batches_per_epoch = tr_data.data_size // batch_size
val_batches_per_epoch = val_data_bad.data_size // batch_size
...
with tf.name_scope("cross_ent"):
    if num_classes > 2 or use_softmax_cross_entropy_loss:
        ...
        #loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=score, weights=y_weight,
        #                                                          labels=y))
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(train_batches_per_epoch):
            ...
            ##################################################
            ### 训练当前batch的数据
            sess.run(train_op, feed_dict={x: img_batch, y_weight: w,
                                          y: label_batch,
        ...
        sess.run(validation_init_op_bad)
        w = np.ones(batch_size, dtype=np.float32)
        ####################################################################################################################
        ### 开始预测
        ####################################################################################################################
        ...
        #
        print('{} Validation Accuracy Bad = {:.4f}, Loss = {:.4f}'.format(datetime.now(),
                                                                      test_acc, test_loss))
        ####################################################################################################################
        ####################################################################################################################
        ### 在验证集上面验证
        sess.run(validation_init_op_good)
        ####################################################################################################################
        ### 开始预测
        ####################################################################################################################
        test_acc = 0.
        ...
        test_loss /= test_count
        #
        print('{} Validation Accuracy Good = {:.4f}, Loss = {:.4f}'.format(datetime.now(),
                                                                      test_acc, test_loss))
