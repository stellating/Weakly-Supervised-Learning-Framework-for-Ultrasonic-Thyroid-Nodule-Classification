...
############################################
checkpoint_path_fc = 'I:/thyroid-all-gray-20180124/trainData/nodules/train_process/finetune_alexnet/model-1.0-1.0-0.5/checkpoints'
...
train_layers = ['fc8', 'fc7', 'fc6', 'conv5', 'conv4', 'conv3'] ## 会更新的层
display_step = 1
...
for iWeight in range(0, len(weightPositives)):
    ...
    #######################################################################
    ### 训练参数
    dataDirThis = rootDir_path + '/model-conv345-' \
    ...
    if not os.path.exists(dataDirThis):
        ...
        print(dataDirThis + ' already exsit!!!')
        exit()
